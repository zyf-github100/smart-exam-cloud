...
- 认证服务默认仅接受 BCrypt 密码；可通过 `ALLOW_LEGACY_PLAIN_PASSWORD=true` 临时开启历史明文兼容迁移窗口，仍需完成全量离线迁移与迁移窗口收敛。
...
- 密码哈希化与认证安全加固。[部分完成，2026-03-06：已支持 BCrypt、默认关闭明文回退并提供迁移开关、JWT 密钥外置与强校验；待完成历史账号离线迁移与密钥托管体系化]。
