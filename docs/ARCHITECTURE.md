...
- MySQL：`MYSQL_HOST`、`MYSQL_PORT`、`MYSQL_URL`、`MYSQL_USERNAME`、`MYSQL_PASSWORD`
...
- 认证服务仅接受 BCrypt 密码；启动阶段会校验 `user_db.sys_user` 不存在历史明文密码记录，并拒绝启用遗留迁移开关。
- `security.jwt.secret` 已要求外置注入并在启动时做强校验（非空、非历史默认值、解码后 >= 32 字节）；仍需纳入统一密钥托管体系。
- 管理员域接口当前仍以 `ADMIN` 角色边界为主，若引入子管理员需补充权限码级校验策略。

建议：

- 在 BCrypt 基线下评估 Argon2 迁移策略。
- 完善权限变更基线审计与最小权限校验。
...
- 密码哈希化与认证安全加固。[已完成，2026-04-12：历史明文密码离线迁移脚本、启动前 BCrypt 校验、灰度开关收口、运维与回滚说明已补齐；Argon2 迁移作为后续增强单独评估]。
